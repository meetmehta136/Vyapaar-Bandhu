...
const ClientDetailPage = () => {
  ...
  if (error || !client) return (
    <AppLayout>
      <div className="flex flex-col items-center justify-center h-64 gap-3">
        <p className="text-destructive-val font-semibold">{error || 'Client not found'}</p>
        <Button variant="indigo" onClick={() => navigate('/clients')}>← Back to Clients</Button>
      </div>
    </AppLayout>
  );
  ...
  const totalItc = clientInvoices.reduce((s: number, i: any) => s + i.itc, 0);
  const totalCgst = clientInvoices.reduce((s: number, i: any) => s + (i.cgst || 0), 0);
  const totalSgst = clientInvoices.reduce((s: number, i: any) => s + (i.sgst || 0), 0);
  const totalIgst = clientInvoices.reduce((s: number, i: any) => s + (i.igst || 0), 0);
  ...
  const complianceStatus = client.complianceStatus || (client.itcThisMonth > 1000 ? 'compliant' : client.itcThisMonth > 0 ? 'attention' : 'at-risk');
  const riskScore = client.riskScore || Math.min(100, Math.round((client.itcThisMonth / 500) * 10 + client.invoiceCount * 5));
  const complianceColor = complianceStatus === 'compliant' ? 'text-success-val' : complianceStatus === 'attention' ? 'text-warning-val' : 'text-destructive-val';
  ...
  return (
    <AppLayout>
      <div className="flex items-center gap-3 mb-6">
        ...
        <span className={cn('text-xs font-semibold capitalize', complianceColor)}>● {complianceStatus}</span>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <KPICard icon="🎯" label="Risk Score" value={`${riskScore}/100`} subtitle={riskScore >= 70 ? 'Low risk' : 'Needs attention'} subtitleColor={riskScore >= 70 ? 'success' : 'warning'} delay={200} />

...
import { createClient, sendWhatsAppReminder, downloadFilingPdf } from '@/lib/api';
...
const ClientsPage = () => {
  ...
  const handleSendReminder = async (client: any) => {
    ...
    const result = await sendWhatsAppReminder(client.phone || '', client.name || '');
    if (result) {
  ...
  const handleDownloadPdf = async (client: any) => {
    await downloadFilingPdf(client.id);
  ...
  return (
    <AppLayout>
      ...
      {error && (
        <div className="mb-6 p-3 rounded-lg bg-destructive/10 border border-destructive/30 text-destructive-val text-xs">
          ⚠️ {error}
        </div>
      )}
